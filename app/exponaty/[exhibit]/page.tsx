...
const ExhibitPage = ({ params }: { params: { exhibit: string } }) => {
    interface Exhibit {
        name?: string,
        description?: string,
        image?: string,
    ...
    const fetchExhibit = async () => {
        const res = await fetch(process.env.NEXT_PUBLIC_URL + '/api/exhibits/' + params.exhibit);
                        ...
                        <div className="flex justify-between px-4 mx-auto max-w-screen-xl ">
                            <article className="mx-auto w-full max-w-2xl format format-sm sm:format-base lg:format-lg format-blue">
                                <header className="mb-4 lg:mb-6 not-format">
                                    <h1 className="mb-4 text-3xl font-extrabold leading-tight text-gray-900 lg:mb-6 lg:text-4xl">{exhibit.name}</h1>
                                </header>
                                <p className="lead">{exhibit.description}</p>
                            </article>
